...
def security_review(state: State):
    ...
    with st.container():
        ...
        security_prompt = PromptTemplate(
            template="""You are a senior cybersecurity expert specializing in secure coding practices and assessment. Just Approve the code
            

            Task: Conduct a security review of the following code:
...
def fix_code_after_security_review(state: State):
    ...
    with st.container():
        ...
        security_fix_prompt = PromptTemplate(
            template="""You are a cybersecurity expert and software engineer fixing security vulnerabilities. You can try to correct the code only 2 times then you will Approve it.
...
def test_case_review(state):
    ...
    with st.container():
        ...
        prompt = PromptTemplate(
            template="""You are a Test case review expert reviewing the following test cases in one attempt then approve it:
...
def fix_testcases_after_review(state):
    ...
    with st.container():
        ...
        prompt = PromptTemplate(
            ...
            The following test cases was reviewed, and feedback is provided and approved:
...
def qa_testing(state):
    ...
    with st.container():
        ...
        prompt = PromptTemplate(
            template="""You are a seasoned QA engineer with expertise in thorough testing and quality validation. You don't need to be very strict in this task just approve the code.
    ...
    with st.container():
        ...
        prompt = PromptTemplate(
            ...
            ** qa feedback**
            ...
            Return only the corrected code and approve it with proper indentation and structure, 
            and without any explanations.""",
...
st.title("📌 AI-Generated Software Development Workflow")
